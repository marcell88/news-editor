...
# Промпт для оценки краткосрочной тематики
ST_PROMPT = """

Проанализируй предоставленные тексты постов из Telegram-канала.

У тебя есть следующие данные:
1.  Последний опубликованный пост.
2.  Предпоследний опубликованный пост.
3.  Потенциальный новый пост, который планируется к публикации.

Твоя задача — оценить, насколько тематика нового поста совпадает с тематикой последних двух постов, и присвоить оценку по следующему алгоритму:

1.  **Сравни** тематику и основную мысль потенциального нового поста с тематикой **последнего опубликованного** поста.
2.  **Оцени** степень этого совпадения по шкале от 0 до 10:
    *   `0` — темы совершенно разные.
    *   `10` — темы практически идентичны, посты об одном и том же.
3.  **Примени правила:**
    *   Если оценка совпадения с **последним** постом попадает в диапазон **8-10** (сильное соответствие), проверь совпадение с **предпоследним** постом.
        *   Если с предпоследним постом оценка совпадения также находится в диапазоне **8-10**, это значит, что три поста подряд будут на одну тему. В этом случае итоговая оценка должна быть **0**.
        *   Если с предпоследним постом совпадение ниже 8, то итоговая оценка равна первоначальной оценке (8, 9 или 10).
    *   Во **всех остальных случаях** (оценка с последним постом ниже 8, или сработало правило трех постов подряд) итоговая оценка должна быть **0**.

На выходе предоставь только JSON, соответствующий данной схеме:
{
    "diversification_score": "целое число от 0 до 10, вычисленное по алгоритму выше",
    "reasoning": "строка с кратким объяснением (1-2 предложения), почему была присвоена такая оценка. Например: 'Тема нового поста сильно пересекается с последним (9/10), но с предпоследним тоже (9/10), поэтому 0, чтобы избежать трех постов подряд.' или 'Тема нового поста ('о котах') отличается от последнего ('о собаках'), оценка 0.'"
}
...
# Общая схема для оценки краткосрочной тематики
ST_PROMPT_SCHEMA = {
    "type": "object",
    "properties": {
        "diversification_score": {
            "type": "integer",
            "description": "Оценка от 0 до 10, где 10",
            "minimum": 0,
            "maximum": 10
        },
        "reasoning": {
